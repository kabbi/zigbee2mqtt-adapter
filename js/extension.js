	...
	class zigbee2mqtt extends window.Extension {
	  ...
	  show() {
			...
			*/
			...
			this.view.querySelector('#extension-zigbee2mqtt-adapter-add-things-button').addEventListener('click', () => {
		...
		parse_body(body){
			...
			if(typeof body.serial_port_path == 'string' && typeof body.radio_serial_port == 'string'){
				...
				if(this.serial_port_name_el){
					this.serial_port_name_el.textContent = body.radio_serial_port.replaceAll('_',' ') + ' (' + body.serial_port_path + ')';
				}
			}
		...
		regenerate_items(items){
			try {
                ...
                if(list.innerHTML == ""){
                    list.innerHTML = '<div style="margin:10rem auto;padding:2rem;max-width:40rem;text-align:center; background-color:rgba(0,0,0,.1);border-radius:10px"><h2>No Zigbee devices paired yet</h2><p>Click on the (+) button if you want to connect a new Zigbee device.</p></div>';
                }
